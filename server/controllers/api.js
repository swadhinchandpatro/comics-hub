...
const getComicsEndpoint = (path) => {
    const timeStamp = new Date().getTime();
    const hash = md5(`${timeStamp}${PRIVATE_API_KEY}${PUBLIC_API_KEY}`);
    return `${COMICS_HOST}/${path}?ts=${timeStamp}&apikey=${PUBLIC_API_KEY}&hash=${hash}`
}

const fetchComics = async () => {
  const response =  await axios.get(getComicsEndpoint('/v1/public/comics'));
  return processResponse.processComicsResponse(response.data.data)
}

const fetchCharacters = async () => {
  const response =  await axios.get(getComicsEndpoint('/v1/public/characters'));
  return processResponse.processCharacterResponse(response.data.data)
}

const fetchComicsOnCharacter = async (characterId) => {
  const response =  await axios.get(getComicsEndpoint(`/v1/public/characters/${characterId}/comics`));
  return processResponse.processComicsResponse(response.data.data)
}
